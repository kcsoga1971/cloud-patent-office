<!-- src/views/services/AmendmentWorkflow.vue -->
<script setup>
...
const statusFilter = ref('all') // 'all' | 'draft' | 'pending' | 'completed'
const sortBy = ref('updated_at') // 'updated_at' | 'created_at' | 'deadline'
...
// ========== 操作 ==========
const createNewCase = () => {
  router.push({ name: 'AmendmentPrep' })
}

const editCase = (jobId) => {
  router.push({ name: 'AmendmentPrep', params: { jobId } })
}
...
const duplicateCase = async (job) => {
  try {
    const newJob = {
      ...
      input_data: {
        ...job.input_data,
        application_number: '', // 清空申請案號
        office_action_number: '', // 清空來函文號
        office_action_date: '',
        deadline: '',
    ...
    alert('✅ 案件已複製')
    router.push({ name: 'AmendmentPrep', params: { jobId: data.id } })
...
<template>
  <div class="amendment-workflow-page">
    
    <!-- 頁面標題 -->
    <div class="page-header">
      <div class="header-content">
        <h1>📝 專利修正案件管理</h1>
        <p class="subtitle">管理您的專利修正申請案件</p>
      </div>
    ...
    <div v-else-if="filteredJobs.length === 0" class="empty-state">
      ...
      <p>{{ searchQuery || statusFilter !== 'all' ? '請嘗試調整搜尋或篩選條件' : '點擊上方按鈕建立您的第一個專利修正案件' }}</p>
      ...
      >
        ...
        <div class="job-content">
          <div class="job-info-grid">
            ...
            <div class="info-item">
              <span class="info-label">回覆期限</span>
